...
function save(){
    ...
    //Storage
    if(fs.existsSync(`C:/Users/${OSname}/Documents/${projName}/files`)){
        if(fs.existsSync(`C:/Users/${OSname}/Documents${projName}/files/${name}.json`)){   
            fs.writeFileSync(`C:/Users/${OSname}/Documents/${projName}/files/${name}.json`,JSON.stringify({name:name, content:storage}));
        }else{
            fs.writeFileSync(`C:/Users/${OSname}/Documents/${projName}/files/${name}.json`,JSON.stringify({name:name, content:storage}));
        }
    }else{
        fs.mkdirSync(`C:/Users/${OSname}/Documents/${projName}/files`);
    }
}
